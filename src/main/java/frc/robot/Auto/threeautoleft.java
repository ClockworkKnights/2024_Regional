...
public class threeautoleft extends SequentialCommandGroup {
  String Path = "two_left";
  String Path1 = "third_left";
  /** Creates a new autonight. */
  public threeautoleft() {
    ...
    addCommands(
      new InstantCommand( ()->{
        if(DriverStation.getAlliance().get()==Alliance.Red)
            ...
            new ShootCmd(RobotContainer.m_shooter, RobotContainer.m_arm, 90.0,ArmConstants.firstarm).withTimeout(2.5),
            RobotContainer.m_robotDrive.followPathCommand(Path).raceWith(new suckin()),
             new ShootCmd(RobotContainer.m_shooter, RobotContainer.m_arm, 90.0,ArmConstants.firstarm).withTimeout(2.5),
             RobotContainer.m_robotDrive.followPathCommand(Path1).raceWith(new suckin()),
              new ShootCmd(RobotContainer.m_shooter, RobotContainer.m_arm, 90.0,ArmConstants.firstarm).withTimeout(2.5)
        //   if(DriverStation.getAlliance().get()==Alliance.Red)
